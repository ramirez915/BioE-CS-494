...
int respPin = A3;
int RESP = 0;                         //--------------------------------------------- NEED THE RESP RATE TO USED TO SEND TO PROCESSING
...
//initialiaze variable of fitness function:
  ...
  while(Serial.read() != 'a') {
    acquire_signal();
   // Serial.println(bpm);                                      // send values to GUI here?????????????????????????
    //Serial.println(r_rate);
    
    // send blue color flag
    sendData(3,4,bpm,r_rate);         // if we are supposed to send data here this is the code  (need to change colorFlag to blue)
